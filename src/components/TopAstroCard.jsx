  ...
  return (
    <div className='lg:px-6 px-3 lg:py-4 py-2 lg:pb-8 pb-4 lg:mx-2 mx-1 lg:my-4 my-1 hover:scale-105 transition-all hover:bg-opacity-90 bg-purple-950 bg-opacity-55 rounded-2xl'>
      ...
      <div className='flex justify-center items-center flex-col'>
        <span className='text-purple-100 text-xs lg:text-base font-medium tracking-wide uppercase'>{info?.name}</span>
        <span className='text-purple-200 opacity-75 text-[9px] lg:text-xs tracking-wide'> ({info?.skill.length > 20 ? `${info?.skill.slice(0,20)}...` : info?.skill })</span>
        {console.log(info)}
      </div>
      </div>
    </div>
